...
def getReviewLength(html):
    reviewVideoreg = r'(<span class="tiny"> Length::)'
    reviewVideore = re.compile(reviewVideoreg)
    reviewVideo_list = re.findall(reviewVideore, html)
    if (len(reviewVideo_list) > 0):
        # reviewPhotoVideo = reviewPhotoVideo_list[0];
        # reviewLength = 'Existing';#leeprince testing
        reviewLengthreg = r'Mins<br /><br /></span>(.*)</div>'
        reviewLengthre = re.compile(reviewLengthreg)
        reviewLength_list = re.findall(reviewLengthre, html)
        if (len(reviewLength_list) > 0):
            # reviewLength = reviewLength_list[0]
            reviewLength = len(reviewLength_list[0])
        else:
            reviewLength = "Unknown"
    else:
        reviewLengthreg = r'<div class="reviewText">(.*)</div>'
        reviewLengthre = re.compile(reviewLengthreg)
        reviewLength_list = re.findall(reviewLengthre, html)
        if (len(reviewLength_list) > 0):
            # reviewLength = reviewLength_list[0]
...
def getReviewPhotoVideo(html):
    reviewPhotoVideoreg = r'(<span class="tiny"> Length::)'
    reviewPhotoVideore = re.compile(reviewPhotoVideoreg)
    reviewPhotoVideo_list = re.findall(reviewPhotoVideore, html)
    if (len(reviewPhotoVideo_list) > 0):
        # reviewPhotoVideo = reviewPhotoVideo_list[0];
        reviewPhotoVideo = 'Existing';
    else:
        reviewPhotoVideoreg = r'(<div style="overflow-x: hidden; overflow-y: hidden;">)'
        reviewPhotoVideore = re.compile(reviewPhotoVideoreg)
        reviewPhotoVideo_list = re.findall(reviewPhotoVideore, html)
        if (len(reviewPhotoVideo_list) > 0):
            # reviewPhotoVideo = reviewPhotoVideo_list[0];
            ...
            reviewPhotoVideo = "Unknown";
    return reviewPhotoVideo
...
def verifyReview(reviewID):
    ...
    if review_page != '':
        try:
            # f = open("./%s" % (reviewID+'.html'), 'w+') # open for 'w'riting
            # f = open(reviewID+'.html', 'w') # open for 'w'riting
            f = open(r'./save_html/'+reviewID+'.html', 'w') # open for 'w'riting && save_html folder location is relative to the php tha calls the python,not the python itself
            f.write(review_page) # write text to file
            f.close() # close the file
            # print('open file is success.')
            reviewPage = 'Insert review page is successful.'
        except IOError as err:
            # print('File Error:'+str(err))
            # print('open file is fail.')
            reviewPage = 'Insert review page is fail.'
    else:
        # print('review_page is NULL.')
        reviewPage = 'review page is NULL.'
    ...
    reviewPhotoVideo   = getReviewPhotoVideo(review_page)
    reviewLength       = getReviewLength(review_page)
    ...
    print reviewASIN+"&&----&%"+reviewTitle+"&&----&%"+reviewStar+"&&----&%"+str(reviewDateFormated)+"&&----&%"+reviewSign+"&&----&%"+shortReviewerID+"&&----&%"+reviewPage+"&&----&%"+reviewPhotoVideo+"&&----&%"+str(reviewLength)
 
if __name__ == '__main__': 
    # reviewID = sys.argv[1]
    # reviewID = "R34P40OBD6JZVC"#server testing ,the corresponding profileID is my_leeprince
    reviewID = "RL30N236IT6YY"#server testing ,the corresponding profileID is my_leeprince
    # reviewID = "R3PRTHFW7KLZIO"#server testing ,the corresponding profileID is my_leeprince
    # reviewID = "R3AY1HS17CWRVL"#server testing ,the corresponding profileID is my_leeprince
